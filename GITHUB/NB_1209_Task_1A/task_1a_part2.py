        ...
        for frame in frame_list:
            if frame_num == frame:
                ...
                for cont in contours:
                    a=np.append(a,cv2.contourArea(cont))
                arr=a
                maxElement=np.amax(arr)
                index=np.where(arr==maxElement)
                (x,y),radius = cv2.minEnclosingCircle(contours[int(np.int32(index))])
                img1=cv2.circle(mask,(int(x),int(y)),int(radius),(0,255,0),2)
                frame_details[frame] = [int(x),int(y)]
